...
Bitcoin.prototype._initDefaults = function (options) {
  ...
  this.maxTransactionHistory =
    options.maxTransactionHistory || Bitcoin.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery =
    options.maxAddressesQuery || Bitcoin.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout =
    options.shutdownTimeout || Bitcoin.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime =
    options.spawnRestartTime || Bitcoin.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Bitcoin.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval =
    options.tryAllInterval || Bitcoin.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval =
    options.startRetryInterval || Bitcoin.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency =
    options.transactionConcurrency || Bitcoin.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress =
    options.zmqSubscribeProgress || Bitcoin.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};
...
Bitcoin.prototype.subscribe = function (name, emitter) {
  ...
  log.info(
    ...
    "bitcoind/" + name,
    "total:",
    this.subscriptions[name].length
  );
};
...
Bitcoin.prototype.subscribeAddress = function (emitter, addresses) {
  ...
  log.info(
    ...
    "total:",
    _.size(this.subscriptions.address)
  );
};
...
Bitcoin.prototype._expandRelativeDatadir = function () {
  if (!utils.isAbsolutePath(this.options.spawn.datadir)) {
    ...
    this.options.spawn.datadir = path.resolve(
      baseConfigPath,
      this.options.spawn.datadir
    );
  }
};
...
Bitcoin.prototype._loadSpawnConfiguration = function (node) {
  ...
  $.checkArgument(
    this.options.spawn,
    'Please specify "spawn" in komodod config options'
  );
  $.checkArgument(
    this.options.spawn.datadir,
    'Please specify "spawn.datadir" in komodod config options'
  );
  $.checkArgument(
    this.options.spawn.exec,
    'Please specify "spawn.exec" in komodod config options'
  );
...
Bitcoin.prototype._checkConfigIndexes = function (spawnConfig, node) {
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn(
      "Reindex option is currently enabled. This means that komodod is undergoing a reindex. " +
...
Bitcoin.prototype._tryAllClients = function (func, callback) {
  ...
  async.retry(
    { times: this.nodes.length, interval: this.tryAllInterval || 1000 },
    retry,
    callback
  );
};
...
Bitcoin.prototype._zmqBlockHandler = function (node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit(
        "bitcoind/hashblock",
...
Bitcoin.prototype._zmqTransactionHandler = function (node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit(
        "bitcoind/rawtransaction",
        message.toString("hex")
      );
    }
    ...
    try {
      ...
      tx.fromString(
        "0400008085202f89000000000000000000000000000000000000000000"
      );
    }
  ...
  if (node._reindex) {
    interval = setInterval(function () {
      node.client.getBlockchainInfo(function (err, response) {
        ...
        log.info(
          "Komodo Daemon Reindex Percentage: " + percentSynced.toFixed(2)
        );
    ...
    fs.readFile(pidPath, "utf8", function (err, pid) {
      ...
      try {
        ...
        if (err && err.code === "ESRCH") {
          log.warn(
            "Unclean komodo process shutdown, process not found with pid: " +
              pid
          );
          return callback(null);
        } else if (err) {
  ...
  self._stopSpawnedBitcoin(function (err) {
    ...
    self.spawn.process.once("exit", function (code) {
      if (!self.node.stopping) {
        ...
        log.warn(
          "Restarting komodo child process in " + self.spawnRestartTime + "ms"
        );
        setTimeout(function () {
  ...
  async.retry(
    ...
    function (done) {
      ...
      node.client = new BitcoinRPC({
        ...
        rejectUnauthorized: _.isUndefined(config.rpcstrict)
          ? true
          : config.rpcstrict,
      });
    ...
    function (err) {
      ...
      if (exitShutdown) {
        return callback(
          new Error("Stopping while trying to connect to komodod.")
        );
      }
  ...
  async.series(
      ...
      function (next) {
        if (self.options.connect) {
          async.map(
            ...
            self._connectProcess.bind(self),
            function (err, nodes) {
              if (err) {
                return callback(err);
              }
    ...
    function (err) {
      ...
      if (self.nodes.length === 0) {
        return callback(
          new Error(
            'Komodo configuration options "spawn" or "connect" are expected'
          )
        );
      }
...
Bitcoin.prototype.getAddressUnspentOutputs = function (
  ...
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool)
    ? true
...
Bitcoin.prototype._getHeightRangeQuery = function (options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError(
        '"end" is expected to be less than or equal to "start"'
      );
    }
    if (clone) {
...
Bitcoin.prototype._getConfirmationsDetail = function (transaction) {
  ...
  if (confirmations < 0) {
    log.warn(
      "Negative confirmations calculated for transaction:",
      transaction.hash
    );
  }
...
Bitcoin.prototype._getAddressDetailsForInput = function (
  input,
  inputIndex,
  result,
  addressStrings
...
Bitcoin.prototype._getAddressDetailsForOutput = function (
  output,
  outputIndex,
  result,
  addressStrings
...
Bitcoin.prototype._getAddressDetailsForTransaction = function (
  transaction,
  addressStrings
) {
  var result = {
  ...
  for (
    ...
    this._getAddressDetailsForOutput(
      output,
      outputIndex,
      result,
      addressStrings
 ...
 * @param {Function} callback
 */
Bitcoin.prototype._getAddressDetailedTransaction = function (
  txid,
  options,
  next
) {
  ...
  self.getDetailedTransaction(txid, function (err, transaction) {
    ...
    var addressDetails = self._getAddressDetailsForTransaction(
      transaction,
...
Bitcoin.prototype._paginateTxids = function (fullTxids, fromArg, toArg) {
  ...
  $.checkState(
    from < to,
    '"from" (' + from + ') is expected to be less than "to" (' + to + ")"
  );
...
Bitcoin.prototype.getAddressHistory = function (addressArg, options, callback) {
  ...
  if (addresses.length > this.maxAddressesQuery) {
    return callback(
      new TypeError(
        "Maximum number of addresses (" + this.maxAddressesQuery + ") exceeded"
      )
    );
  }
  ...
  function querySummary() {
    async.parallel(
        ...
        function getMempool(done) {
          ...
          self.client.getAddressMempool(
            ...
            function (err, response) {
              ...
              summary.unconfirmedBalance = self._getBalanceFromMempool(
                response.result
              );
              done();
            }
      ...
      function (err) {
        ...
        var availableTxids = mempoolTxids.reverse().concat(summaryTxids);
        summary.txidOverflow =
          availableTxids.length > self.maxTxids ? true : false;
        summary.availableTxids = !summary.txidOverflow ? availableTxids : [];
...
Bitcoin.prototype._maybeGetBlockHash = function (blockArg, callback) {
  ...
  if (
    _.isNumber(blockArg) ||
    (blockArg.length < 40 && /^[0-9]+$/.test(blockArg))
  ) {
  ...
  function addOutputsToTx(tx, result) {
    ...
    for (var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      ...
      if (
        ...
        out.scriptPubKey.addresses.length === 1
      ) {
        ...
        [
          function (callback1) {
            client.getRawTransaction(txid, 1, function (err, response) {
              ...
              if (!tx.coinbase) {
                tx.feeSatoshis =
                  tx.inputSatoshis - tx.outputSatoshis + netJoinSplitZatoshis;
              } else {
                tx.feeSatoshis = 0;
              }
              ...
              if (tx.fOverwintered && tx.version >= 4) {
                ...
                tx.feeSatoshis =
                  tx.feeSatoshis +
                  (isNaN(result.valueBalanceZat)
                    ? result.valueBalance * 1e8
                    : result.valueBalanceZat);
                /* 
          ...
          */
          ...
          function (callback2) {
            ...
            for (let i = 0; i < vins.length; i++) {
              functionsArray.push(function (callback3) {
                var prevTxId = vins[i].prevTxId || null;
                var rewards = 0;
                if (prevTxId) {
            ...
            async.parallel(functionsArray, function (err, res) {
              var totalReward = 0;

              for (let i = 0; i < res.length; i++) {
                tx.inputs[i].rewardClaimed = res[i].rewards
                  ? res[i].rewards * 1e-8
                  : 0;

                var actualTimeRewardAccrued = res[i].prevTxnLocktime
                  ? (tx.blockTimestamp || self.tipTime) - res[i].prevTxnLocktime
                  : null;
                if (actualTimeRewardAccrued) {
                  tx.inputs[i].timeRewardAccrued =
                    actualTimeRewardAccrued < 31 * 24 * 60 * 60
                      ? actualTimeRewardAccrued
                      : 31 * 24 * 60 * 60;
                } else {
                  tx.inputs[i].timeRewardAccrued = null;
                }

                totalReward = tx.inputs[i].rewardClaimed + totalReward;
              }
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once("exit", function (code) {
      if (!exited) {
        ...
        if (code !== 0) {
          var error = new Error(
            "komodod spawned process exited with status code: " + code
          );
          error.code = code;
